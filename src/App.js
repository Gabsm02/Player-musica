...
function App() {
  ...
  function handlePrevious() {
    const previousIndex = (currentMusicIndex - 1 + musics.length) % musics.length;
    handlePlay(previousIndex);
  }
    ...
    <div className="container">
      ...
      <div className='main'>
        ...
        {musics.map((music, index) => (
          <a key={index} onClick={() => handlePlay(index)}>
            <MusicCard
              className='botao'
              foto={music.cover}
              titulo={music.title}
      ...
      <div className='music-bar'>
          <div className='player-buttons'>
              ...
              <img  onClick={handlePrevious} className='previous' src={Previous} alt='Previous'/>


              {isPlaying ? (
              
                <img onClick={handlePause} src={Pause} alt='Pause' />
              ) : (
                <img onClick={() => handlePlay(0)} src={Play} alt='Play'/>
              )}
            ...
            <div className='bar-container' onClick={handleBarClick}>
                <div className='bar' style={{ width: `${(currentTime / duration) * 100}%` }}></div>
                <div className='point' style={{ left: `${(currentTime / duration) * 100}%` }}></div>
              </div>
